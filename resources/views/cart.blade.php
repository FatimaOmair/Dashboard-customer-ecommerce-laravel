                        ...
                        <tr>
                            <td>
                                <a href="{{ route('shop.product.details', ['slug' => $item->product->slug]) }}">
                                    <img src="{{ asset('storage/' . $item->product->image) }}" class="blur-up lazyloaded" alt="{{ $item->product->name }}">
                                </a>
                            </td>
                            <td>
                                <a href="{{ route('shop.product.details', ['slug' => $item->product->slug]) }}">{{ $item->product->name }}</a>
                                <div class="mobile-cart-content row">
                                    <div class="col">
                                        <div class="qty-box">
                                            <div class="input-group">
                                                <input type="number" name="quantity" data-id="{{ $item->id }}" class="form-control input-number" onchange="updateQuantity(this)" value="{{ $item->quantity }}">
                                            </div>
                                        </div>
                                    </div>
                                    <div class="col">
                                        <h2>${{ $item->product->regular_price }}</h2>
                                    </div>
                                    <div class="col">
                                        <h2 class="td-color">
                            ...
                            <td>
                                <h2 class="td-color item-subtotal">${{ $item->quantity * $item->product->regular_price }}</h2>
                            </td>
                            <td>
                                <form method="POST" action="{{ route('cart.remove') }}" class="remove-item-form">
                                    @csrf
                            ...
                            <div class="cart-box-details">
                                <div class="total-details">
                                    <div class="top-details">
                                        <h3>Cart Totals</h3>
                                        <h6>Sub Total <span id="cart-subtotal">${{ $cartItems->sum(function($item) { return $item->quantity * $item->product->regular_price; }) }}</span></h6>
                                        <h6>Tax <span id="cart-tax">${{ $cartItems->sum(function($item) { return $item->quantity * $item->product->regular_price; }) * 0.1 }}</span></h6>
                                        <h6>Total <span id="cart-total">${{ $cartItems->sum(function($item) { return $item->quantity * $item->product->regular_price; }) * 1.1 }}</span></h6>
                                    </div>
